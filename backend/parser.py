...
def recursive_substitute(expr, subs):
    ...
    替换成其对应的表达式，并继续递归替换。
    如果 subs 中的值不是字符串，则将其转换为字符串。
    如果 expr 本身不是字符串，也转换为字符串。
    """
    ...
    while prev_expr != expr:
        prev_expr = expr
        expr = pattern.sub(
            lambda m: subs[m.group(0)] if isinstance(subs[m.group(0)], str) 
            else str(subs[m.group(0)]),
            expr
        )
    return expr

def process_indicators_RL(data):
    """
...
def process_evaluation_RL(data):
    """
    将 evaluation 部分转换为：
      {
...
def process_data(data):
    ...
    """
    indicators = process_indicators_RL(data)
    evaluation = process_evaluation_RL(data)
